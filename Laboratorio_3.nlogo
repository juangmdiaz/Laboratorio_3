...
to revisar-vecinos
  let vecinos count (turtles-on neighbors) with [color = white]
    if-else vecinos > 0 [
      fd -1
      ][]
end
...
to aumentar-edad
  set Edad Edad + random (4 - 1) + 1
end

to consumir-alimento
  if pcolor = green [
  set Energia Energia + random (3 - 2) + 2  ]
end
